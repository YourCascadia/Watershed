var padding, margin, docwidth, docheight, canvaswidth, canvasheight, buttonwidth,
            a, b, goldenratio = 1.61803399, halfbuttonwidth, buttonPosRight, buttonPos33, buttonPosLeft, buttonwidth;
...
$(document).ready(function () {
    ...
    */
    ...
    //function to center selected item vertically and horizontally
    (function ($) {
        //animation controls the speed of the animation. If it's blank, it will be instant
        //bouncepast controls the ability for the selected item to bounce past its destination and back
        $.fn.centerVH = function (animation, bouncepast) { 
            var width, height, pw, ph, x, y, xv, xy;
            var w = $(this).width(); // w is the width of the selected item
            var h = $(this).height(); // h is the height
            var p = $(this).parent(); // p is the parent item
            var pt = parseInt($(this).css("padding-top")); // pt is the numerical value for the top padding
            var pl = parseInt($(this).css("padding-left")); // pl is padding left
            var pr = parseInt($(this).css("padding-right")); // pr is padding right
            var pb = parseInt($(this).css("padding-bottom")); // pb is padding bottom
            var pvh = ((pt + pb) / 2); // pvh is the total vertical padding
            var pwh = ((pl + pr) / 2); // pwh is the total horizontal padding

            if (p) { //if the selected item has a parent
                pw = $(this).parent().width(); //pw is the parent width
                ph = $(this).parent().height(); //ph is the parent height
            } else { pw = window.width(); ph = window.height(); } //otherwise, use the window height

            x = ((pw / 2) - (w / 2) - pwh); // x is the left value (in pixels)
            y = ((ph / 2) - (h / 2) - pvh); // y is the top value (in pixels)

            if (x<=0 || y<=0) { // if x or y is less than or equal to 0 
                xv = "100%"; xy = "100%"; // make the width 100%
                if (animation) { // if there is an animation time, animate the width change
                    $(this).animate({width: "100%", height: "100%"}, animation);
                } else { //otherwise, set the width instantly

                    $(this).css({width: "100%", height: "100%"}); 
                } 
            } 
            else { // if x and y are both at or above 0;
                xv = x; xy = y; 
                if (animation) { // if an animation time is set, 
                    $(this).animate({ marginTop: xy, marginLeft: xv }, animation); // animate the height and width
                    if (bouncepast) { // if bouncepast is set, animate past the calculated position and back
                        var fa = animation / 50;
                        var fx = x / 50;
                        var fy = y / 50;
                        x = fx + x;
                ...
                } else { // otherwise, set the position instantly (note: this is the default action);
    ...
    (function ($) {
        $.fn.continueLoading = function (speed, state) {
            speed = speed || 500;
            state = state || "default";
    ...
    (function ($) {
        ...
        $("#contextmenu").draggable({ cancel: ".innercontent", cancel: ".ad" });
        ...
        (function ($) {
            $.fn.sizes = function () {
                //PADDING
                padding = 12, margin = 12, docwidth = $("body").width(), docheight = $("body").height(), canvaswidth = docwidth - ((2 * padding) + (2 * margin)), canvasheight = docheight - ((2 * padding) + (2 * margin));
                
                $(".canvas").width(canvaswidth), $(".canvas").height(canvasheight);
                buttonwidth = $("#button").width(); //buttonwidth = the width of the button;

                a = canvaswidth / goldenratio, b = canvaswidth - a - 1, halfbuttonwidth = buttonwidth / 2, buttonPosRight = (canvaswidth + margin + padding - halfbuttonwidth), buttonPos33 = b + margin + padding - halfbuttonwidth, buttonPosLeft = margin + padding + 10;
        ...
        $(".ad").click(function(){
            var populate = $(".ad").prop('outerHTML');
            var pt = populate.toString();
            var r = "<div class=\"ad";
            var pr = pt.replace(r, "<div class=\"ad exposed");
            var b = "<div class=\"xbutton\"><h3>X</h3></div>";
            var e = "</div>";
            var i = "<input type=\"text\" ng-model=\"search.item\"/>";
            var p = b + pr;
            // $(document).adPopUp(populate);
            // $( "#contextmenu" ).html(p);
            $(document).mainMenu(p);
            $("#contextmenu").css({backgoundColor: "#333"});
            $("#contextmenu .ad").addClass("exposed");
            $("#contextmenu").width(300);
            
            //removing previous popup. adding new, bigger popup
            return populate;
        });
